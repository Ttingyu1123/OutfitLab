...
const AnalysisModal: React.FC<AnalysisModalProps> = ({ isOpen, onClose, result, isLoading, lang }) => {
  ...
  // Filter out the JSON block for display purposes
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fade-in">
      ...
      >
        ...
        <div className="px-6 py-5 border-b border-slate-100 flex items-center justify-between bg-gradient-to-r from-brand-50 to-white rounded-t-3xl">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">{t.modal_subtitle}</p>
            </div>
          </div>
          <button 
            onClick={onClose}
          ...
          {isLoading ? (
             <div className="flex flex-col items-center justify-center py-12 space-y-4">
                ...
                <div className="text-center">
                   ...
                   <p className="text-slate-400 text-sm mt-1">{t.modal_loading_desc}</p>
                </div>
             </div>
          ) : result ? (
             <div className="prose prose-slate prose-sm md:prose-base max-w-none">
               <div className="bg-brand-50/50 p-6 rounded-2xl border border-brand-100 text-slate-700 leading-relaxed whitespace-pre-wrap font-medium">
                 {/* Simple rendering of markdown-like text */}
                 {displayContent.split('\n').map((line, i) => {
                   if (line.startsWith('**') || line.startsWith('##') || line.includes('分析') || line.includes('Analysis') || line.includes('建議') || line.includes('Tips')) {
                     return <p key={i} className="font-bold text-brand-800 text-lg mt-4 mb-2">{line.replace(/\*\*/g, '')}</p>;
                   }
        ...
        <div className="px-6 py-4 border-t border-slate-100 bg-slate-50 rounded-b-3xl flex flex-col gap-3">
          {/* Helpful Tip about the hidden JSON data */}
          {!isLoading && result && (
